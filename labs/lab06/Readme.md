...
Коммутатор S1 стал корневым мостом потому что его идентификатор (BID) наименьший. Так как приоритет моста и расширенный идентификатор (VLAN) у каждого коммутатора одинаков, то корневым мостом становится коммутатор с наименьшим MAC-адресом.

Какие порты на коммутаторе являются корневыми портами?
На коммутаторе S1 нет корневых портов. Корневые порты расположены на коммутаторе S2(f0/2), а так же на S3(f0/4).

Какие порты на коммутаторе являются назначенными портами?
На коммутаторе S1 все порты - назначенные. Также, назначенным портом является f0/2 на коммутаторе S3.

Какой порт отображается в качестве альтернативного и в настоящее время заблокирован?
Альтернативным является порт f0/4 на коммутаторе S2.

Почему протокол spanning-tree выбрал этот порт в качестве невыделенного (заблокированного) порта?
Порт f0/4 коммутатора S2 выбран в качестве невыделенного (заблокированного) порта потому что у коммутатора S2 самый высокий BID. В данном случае происходит сравнение по BID коммутаторов потому что стоимости пути до корневого моста от коммутатора S2 и S3 равны (19). Другая сторона линка заблокированного порта не может быть корневой - это всегда назначенный порт.   
...
Назначенным портом в данном случае будет являться тот порт, стоимость пути к корневому мосту которого наименьшая. Благодаря тому, что мы уменьшили метрику корневого порта f0/2 коммутатора S2, то его его порт f0/4 автоматически стал назначенным, а альтернативным стал порт f0/2 коммутатора S3, стоимость пути к корневому мосту которого на 1 больше. 
...
- #### Какой порт выбран протоколом STP в качестве порта корневого моста на каждом коммутаторе некорневого моста?
 На коммутаторе S2 это порт f0/1, а на S3 это f0/3.
- #### Почему протокол STP выбрал эти порты в качестве портов корневого моста на этих коммутаторах? 
Данные порты были выбраны в качестве портов корневого моста благодаря тому, что у них минимальная стоимость пути до корневого моста. Так как у портов f0/1 и f0/3 коммутатора S2 стоимость одинаковая и нужно определиться с выбором корневого порта между двумя равноправными портами, то выбирается порт с наименьшим порядковым номером.
